...
if menu == "📄 Project Proposal":
    ...
    st.subheader("1. හැඳින්වීම (Introduction)")
    st.write(
    ...
    st.subheader("2. ව්‍යාපෘතියේ අරමුණු (Objectives)")
    st.markdown("""
    * 🚗 වාහන නඩත්තු වියදම් නිවැරදිව වාර්තා කරගැනීම.
    * ⛽ ඉන්ධන කාර්යක්ෂමතාව (Fuel Efficiency) ගණනය කර පාලනය කරගැනීම.
    ...
    * 🌐 ශ්‍රී ලංකේය වාහන ප්‍රජාව සඳහා ඩිජිටල් ප්‍රජා මධ්‍යස්ථානයක් (Community Hub) නිර්මාණය කිරීම.
    ...
    st.subheader("3. අපේක්ෂිත ප්‍රතිලාභ (Expected Benefits)")
    ...
    st.subheader("4. සම්බන්ධ වීමට / අනුග්‍රහය දැක්වීමට (Sponsorship & Contact)")
...
# Check if vehicles exist for management
if not st.session_state.vehicles and menu != "📄 Project Proposal":
    st.warning("⚠️ කරුණාකර පළමුව වම්පස මෙනුවෙන් **'Add Vehicle'** හරහා ඔබේ වාහනයක් එකතු කරගන්න.")
elif st.session_state.vehicles:
    selected_vehicle = st.sidebar.selectbox("ניהול වාහනය තෝරන්න (Select Vehicle):", st.session_state.vehicles)

    # ==========================================
    # 3. ADD FUEL / EXPENSE LOG
    # ==========================================
    if menu == "⛽ Add Fuel/Service Log":
        st.subheader(f"⛽ {selected_vehicle} - වියදම් සහ ඉන්ධන දත්ත ඇතුළත් කිරීම")
        with st.form("expense_form"):
            category = st.selectbox("වර්ගය:", ["Fuel", "Full Service", "Tyre Change", "License/Insurance", "Repairs", "Other"])
    ...
    # ==========================================
        ...
        st.subheader(f"📊 {selected_vehicle} - වියදම් සාරාංශය සහ විශ්ලේෂණය")
        ...
        if len(st.session_state.expenses) > 0:
            ...
            if not v_df.empty:
                ...
                st.markdown("### 📋 වියදම් ඉතිහාසය")
                st.dataframe(v_df, use_container_width=True)
                
                # Chart
                st.markdown("### 📈 වියදම් ප්‍රස්තාරය")
                fig, ax = plt.subplots()
                v_df.groupby("Category")["Cost (LKR)"].sum().plot(kind="bar", ax=ax, color=["#ff9999","#66b3ff","#99ff99","#ffcc99"])
